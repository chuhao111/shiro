...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    public int updateMenu(Menu menu) throws Exception {
        ...
        setMenu(menu);
        int updateById = baseMapper.updateById(menu);
    ...
    @Override
    @Transactional
    public boolean deleteMeuns(String[] menuIds) throws Exception {
    	boolean deleteMenus = false;
        for (String menuId : menuIds) {
            // 查找与这些菜单/按钮关联的用户
            List<String> userIds = this.baseMapper.findUserIdsByMenuId(String.valueOf(menuId));
            // 递归删除这些菜单/按钮
           deleteMenus = this.baseMapper.deleteMenus(menuId);
            // 重新将这些用户的角色和权限缓存到 Redis中
            this.userManager.loadUserPermissionRoleRedisCache(userIds);
