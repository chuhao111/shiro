...
*/
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        if (StringUtils.isBlank(encryptTokenInRedis))
            throw new AuthenticationException("登录状态已经失效");//token过期
        /*return JsonUtils.objectToJson(StateResultUtil.build(400, errorMessage));*/

        String username = JWTUtil.getUsername(token);

        if (StringUtils.isBlank(username))
            throw new AuthenticationException("登录状态已经失效");//token校验不通过

        // 通过用户名查询用户信息
        User user = userManager.getUser(username);

        if (user == null)
            throw new AuthenticationException("登录状态已经失效");//用户名或密码错误
